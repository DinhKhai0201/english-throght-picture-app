...
function arpabetToIPAString(arpabet) {
  return arpabet
    ...
    .map((token) => {
      ...
      const exact = arpabetToIpa(normalized);
      const exactIpa = extractIpa(exact, normalized);
      if (exactIpa) return exactIpa;

      const withoutStress = normalized.replace(/[0-9]/g, "");
      const fallback = arpabetToIpa(withoutStress);
      return extractIpa(fallback, withoutStress) || withoutStress;
    })
    .join("");
}

function extractIpa(result, key) {
  if (!result) return "";
  if (typeof result === "string") {
    return /No entry found/i.test(result) ? "" : result;
  }

  const value = result[key];
  if (!value) return "";
  if (typeof value === "string") {
    return /No entry found/i.test(value) ? "" : value;
  }

  return value.ipa_no_space || value.ipa || "";
}
